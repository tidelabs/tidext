...
async fn tx_basic_transfer() -> Result<(), Error> {
  // 10 tides
  let transfer_amount = Asset::Tifi.saturating_mul(10);
  ...
  let alice_pre = client
    .balance(alice.account_id(), Asset::Tifi.currency_id())
    .await?
    .available;

  let charlie_pre = client
    .balance(charlie.account_id(), Asset::Tifi.currency_id())
    .await?
    .available;
